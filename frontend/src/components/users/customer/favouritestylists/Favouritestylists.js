...
const Favouritestylists = () => {
  ...
  useEffect(() => {
    ...
    axiosInstance
      .post("cust/all-favourites/", datas)
      .then((response) => {
        setAllFavourites(response.data.allfavourites);
        console.log(response.data.allfavourites);
      })
      .catch((error) => {
        alert(error);
      });
  ...
  return (
    ...
    >
      <CardContent sx={{ alignItems: 'center', textAlign: 'center' }}>
        <Avatar src={item.beautician.image  }sx={{ '--Avatar-size': '4rem' }} />
        <Chip
          size="sm"
          variant="soft"
          color="primary"
